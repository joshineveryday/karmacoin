Karmacoin Core version 0.18.0 is now available from:

  <https://karmacoincore.org/bin/karmacoin-core-0.18.0/>
...
`/Applications/Karmacoin-Qt` (on Mac) or `karmacoind`/`karmacoin-qt` (on
Linux).
...
Karmacoin Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Karmacoin Core on unsupported systems.

Karmacoin Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, Karmacoin Core does not yet change appearance when
...
`contrib/init/karmacoind.service`, it has been changed to use
`/var/lib/karmacoind` as the data directory instead of
`~karmacoin/.karmacoin`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/karmacoind` will
exist has enough space (check using `df -h /var/lib/karmacoind`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/karmacoin/karmacoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://karmacoincore.org/en/contact/) the Karmacoin Core
...
The systemd init file (`contrib/init/karmacoind.service`) has been changed
to use `/var/lib/karmacoind` as the data directory instead of
`~karmacoin/.karmacoin`. This change makes Karmacoin Core more consistent with
...
`datadir` option in `/etc/karmacoin/karmacoin.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/karmacoin/karmacoin.conf`.


Documentation
-------------

- A new short [document](https://github.com/karmacoin/karmacoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/karmacoin/karmacoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/karmacoin/karmacoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/karmacoin/karmacoin/blob/master/doc/karmacoin-conf.md)
  about the `karmacoin.conf` file describes how to use it to configure
  Karmacoin Core.

- A new document introduces Karmacoin Core's BIP174 [Partially-Signed
  Karmacoin Transactions
  (PSBT)](https://github.com/karmacoin/karmacoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/karmacoin/karmacoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Karmacoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Karmacoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/karmacoin/karmacoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/karmacoin/karmacoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Karmacoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Karmacoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/karmacoin/karmacoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/karmacoin/karmacoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart karmacoind with the `-deprecatedrpc=generate`
...
- A new `karmacoin-wallet` tool is now distributed alongside Karmacoin
...
This section describes planned changes to Karmacoin Core that may affect
other Karmacoin software and services.

- Since version 0.16.0, Karmacoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Karmacoin Core 0.20 (expected about
  a year after 0.18), Karmacoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Karmacoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Karmacoin Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Karmacoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Karmacoin Core
...
- A sub-project of Karmacoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Karmacoin Core.  See their [project
  page](https://github.com/karmacoin-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Karmacoin Core's own implementation, although entropy
  gathered by Karmacoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Karmacoin Core a
...
- On macOS, Karmacoin Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `karmacoin_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libkarmacoin_server.a` (jnewbery)
...
- #13926 [Tools] karmacoin-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like karmacoind (MarcoFalke)
...
- #14521 qt, docs: Fix `karmacoin-qt -version` output formatting (hebasto)
...
- #11625 Add KarmacoinApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libkarmacoin_server` (laanwj)
...
- #15154 Configure: karmacoin-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/karmacoin-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Karmacoin Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libkarmacoin_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in karmacoin-util-test.py (isghe)
...
- #12775 Integration of property based testing into Karmacoin Core (Christewart)
...
- #12246 Bugfix: Only run karmacoin-tx tests when karmacoin-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_karmacoin` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_karmacoin` (MarcoFalke)
...
- #14186 karmacoin-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_karmacoin.conf` message on startup if conf file exists (leishman)
...
- #12255 Update karmacoin.service to conform to init.md (dongcarl)
...
- #14497 Add doc/karmacoin-conf.md (hebasto)
...
- #15354 Add missing `karmacoin-wallet` tool manpages (MarcoFalke)
...
- #15765 correct karmacoinconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support karmacoin folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/karmacoin/).
