...
const char * const KARMACOIN_CONF_FILENAME = "karmacoin.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "karmacoind -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Karmacoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Karmacoin
    // Mac: ~/Library/Application Support/Karmacoin
    // Unix: ~/.karmacoin
#ifdef WIN32
    // Windows
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Karmacoin";
#else
    // Unix
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", KARMACOIN_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Karmacoin Core copyright is not removed by accident
    if (copyright_devs.find("Karmacoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Karmacoin Core developers";
    }
